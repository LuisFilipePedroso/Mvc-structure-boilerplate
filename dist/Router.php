...
class CreateRouter {
    ...
    public function generateRouterClass($path, $filename) {
        $router = fopen($path . "/" . $filename . ".php", "w") or die("Unable to open file!");
        $line = fwrite($router, "<?php\n");
        $line = fwrite($router, "class Router {
        private \$routes = [];
